...
export function SchedulePage() {
  ...
  const [form, setForm] = useState({
    user_id: '', site: profile?.site ?? SITES[0],
    start_time: '', end_time: '',
    shift_type: 'day' as 'day' | 'night', notes: '',
  });
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('schedules').insert({
        user_id: form.user_id || user!.id, site: form.site,
        start_time: form.start_time, end_time: form.end_time,
        shift_type: form.shift_type, notes: form.notes || null, created_by: user!.id,
      });
  ...
  const prevWeek = () => { const d = new Date(week); d.setDate(d.getDate() - 7); setWeek(d.toISOString().split('T')[0]); };
  const nextWeek = () => { const d = new Date(week); d.setDate(d.getDate() + 7); setWeek(d.toISOString().split('T')[0]); };
  ...
  return (
    <div className="space-y-5">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nouvelle vacation" size="md">
        <div className="space-y-4">
          {agents && (
            <div>
              <label className="label-base">Agent</label>
              <select value={form.user_id} onChange={(e) => setForm({ ...form, user_id: e.target.value })} className="select-base">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select value={form.shift_type} onChange={(e) => setForm({ ...form, shift_type: e.target.value as 'day' | 'night' })} className="select-base">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="datetime-local" value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })} className="input-base" />
            </div>
            <div>
              <label className="label-base">Fin</label>
              <input type="datetime-local" value={form.end_time} onChange={(e) => setForm({ ...form, end_time: e.target.value })} className="input-base" />
            </div>
          </div>
          <div>
            <label className="label-base">Notes</label>
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2} className="input-base resize-none" placeholder="Informations complémentaires..." />
          </div>
          <div className="flex justify-end gap-2 pt-1">
            <Button variant="ghost" onClick={() => setModalOpen(false)}>Annuler</Button>
            <Button onClick={() => createMutation.mutate()} loading={createMutation.isPending} disabled={!form.start_time || !form.end_time}>
              Enregistrer
            </Button>
          </div>
        </div>
