  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-[18px] font-bold text-zinc-50 tracking-tight">Incidents</h1>
      ...
      {/* Filter tabs */}
      <div className="flex items-center gap-1 bg-zinc-950 border border-zinc-800 rounded-lg p-1 w-fit">
        {FILTERS.map((f) => (
          <button
            key={f.key}
            ...
            className={[
              ...
              filter === f.key
                ? 'bg-zinc-900 text-zinc-200 shadow-card'
      ...
      <div className="border border-white/[0.06] rounded-xl overflow-hidden">
        {/* Table header */}
        <div className="hidden sm:grid sm:grid-cols-[16px_1fr_100px_84px_84px_72px_auto] gap-3 px-4 py-2.5 border-b border-white/[0.06] bg-white/[0.015]">
          <span />
          <span className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.08em]">Incident</span>
          <span className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.08em]">Site</span>
          <span className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.08em]">Priorité</span>
          <span className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.08em]">Statut</span>
          <span className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.08em]">Date</span>
          <span />
        </div>
            ...
            {incidents?.map((incident) => {
              ...
              return (
                <div
                  ...
                  className="group flex sm:grid sm:grid-cols-[16px_1fr_100px_84px_84px_72px_auto] items-center gap-3 px-4 py-3.5 hover:bg-white/[0.02] transition-colors cursor-pointer"
                >
                  {/* Priority dot */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {incident.description && (
                      <p className="text-[12px] text-zinc-600 truncate mt-0.5 hidden sm:block">
                        {incident.description}
                      </p>
                    )}
                  </div>
                  ...
                  {/* Date */}
                  <span className="hidden sm:block text-[11px] text-zinc-600 font-mono tabular-nums">{date}</span>
                    ...
                    >
                      {incident.status === 'open' && (
                        <button
                          ...
                          className="text-[11px] font-medium text-zinc-400 hover:text-blue-400 transition-colors px-2 py-1 rounded hover:bg-blue-500/10"
                        >
                          Prendre
                        </button>
                      )}
                      <button
                        ...
                        className="text-[11px] font-medium text-zinc-400 hover:text-emerald-400 transition-colors px-2 py-1 rounded hover:bg-emerald-500/10"
                      >
                        Résoudre
                      </button>
                    </div>
      ...
      >
        {detailIncident && (
          <div className="space-y-4">
            ...
            {/* Title */}
            <div>
              <p className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.08em] mb-1">Incident</p>
              <p className="text-[15px] font-semibold text-zinc-100">{detailIncident.title}</p>
            </div>
            ...
            {detailIncident.description && (
              <div>
                ...
                <p className="text-[13px] text-zinc-300 leading-relaxed">{detailIncident.description}</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg px-3 py-2.5">
                <p className="text-[11px] text-zinc-600 uppercase tracking-[0.08em] mb-0.5">Site</p>
                <p className="text-[13px] font-medium text-zinc-300">{detailIncident.site}</p>
              </div>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg px-3 py-2.5">
                <p className="text-[11px] text-zinc-600 uppercase tracking-[0.08em] mb-0.5">Signalé par</p>
                <p className="text-[13px] font-medium text-zinc-300">{detailIncident.reporter?.full_name ?? '—'}</p>
              </div>
              <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg px-3 py-2.5">
                <p className="text-[11px] text-zinc-600 uppercase tracking-[0.08em] mb-0.5">Signalé le</p>
              ...
              {detailIncident.assignee && (
                <div className="bg-white/[0.02] border border-white/[0.04] rounded-lg px-3 py-2.5">
                  <p className="text-[11px] text-zinc-600 uppercase tracking-[0.08em] mb-0.5">Assigné à</p>
                  <p className="text-[13px] font-medium text-zinc-300">{detailIncident.assignee.full_name}</p>
                </div>
              )}
            ...
            {detailIncident.photo_url && (
              <div>
                ...
                <img
                  ...
                  className="w-full max-h-64 object-cover rounded-xl border border-white/[0.06]"
                />
              </div>
            )}
          ...
          <div>
            ...
            {photoPreview ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-4 border border-dashed border-white/[0.08] rounded-lg hover:border-white/[0.15] hover:bg-white/[0.02] transition-all text-zinc-600 hover:text-zinc-400"
              >
